...
module.exports = {
  ...
  async execute(message, args, client) {
    const user = message.mentions.members.first() || message.guild.users.cache.get(args[0]) || message.guild.members.cache.filter((m) => m.user.tag.includes(args.join(' '))).first();
    if (!user) {
