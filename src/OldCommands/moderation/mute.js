...
module.exports = {
  ...
  async execute(message, args, client) {
    const user = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.filter((m) => m.user.tag.includes(args.join(' ')) || m.displayName.includes(args.join(' ')) || m.user.username.includes(args.join(' '))).first();
    ...
    if (!args[1]) return message.errorMessage('Veuillez fournir une durée de 14 jours ou moins (1s/m/h/d)');
    const time = ms(args[1]);
    if (!time || time > 1209600000) return message.errorMessage('Veuillez fournir une durée de 14 jours ou moins (1s/m/h/d)');
