...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    const action = args[1];
    if (action === '+') {
      const role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2]) || message.guild.roles.cache.filter((m) => m.name.includes(args.slice(2).join(' '))).first();
      ...
      member.roles.add(role.id).catch(((error) => message.errorMessage(`Une erreur est suvenue lorsque j'ai essayé d'ajouter le rôle ${role} à ${member}`)));
      ...
      member.roles.remove(role.id).catch(((error) => message.errorMessage(`Une erreur est suvenue lorsque j'ai essayé d'ajouter le rôle ${role} à ${member}`)));
