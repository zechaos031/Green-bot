...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    } else if (!response.invites && !response.messages) {
    ...
    async function getResponses(message) {
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        if (i === 0) {
          const channel = m.mentions.channels.first() || message.guild.channels.cache.get(content);
          if (channel) {
        ...
        if (i === 4) {
          ...
          } else if (isNaN(content) || content < 1 || m.content.includes('-') || m.content.includes('+') || m.content.includes(',') || m.content.includes('.')) {
