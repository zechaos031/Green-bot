...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args[0] === 'add') {
      try {
        const role = message.mentions.roles.first() || message.guild.roles.cache.get(args[1]) || message.guild.roles.cache.filter((m) => m.name.includes(args.join(' '))).first();
        ...
        message.channel.send(embed).then((m) => {
          guild.members.fetch().then((members) => {
            const memberCount = members.filter((u) => !u.roles.cache.has(role.id)).size;
            members.filter((u) => !u.roles.cache.has(role.id)).forEach((membera) => {
              u += 1;
              let embed2;
              if (u == memberCount) {
                const time = message.guild.memberCount * 500;
