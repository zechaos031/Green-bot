...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    const warndb = await Warn.find({ serverID: message.guild.id, manID: member.id });
    if (warndb) {
      const verynew = new Warn({
        ...
        member.send(`Bonjour ${member.user.tag}, Vous avez été avertit sur ${message.guild.name} pour la raison ${reason}.`).catch(() => message.errorMessage(`Je n'ai pas pu MP ${member.user.tag} mais le warn a bien été enregistré. `));
      ...
      const verynew = new Warn({
        ...
        member.send(`Bonjour ${member.user.tag}, Vous avez été avertit sur ${message.guild.name} pour la raison ${reason}.`).catch(() => message.errorMessage(`Je n'ai pas pus MP ${member.user.tag} mais le warn a bien été enregistré. `));
