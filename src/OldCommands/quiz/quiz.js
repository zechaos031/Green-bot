...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (verify) {
      ...
      const find = await QuizBD.findOne({ userID: message.author.id });
      if (find) {
          ...
          message.channel.awaitMessages((m) => m.author.id === message.author.id, { max: 1, time: 30000, errors: ['time'] })
            .then(async (collected) => {
              ...
              if (quiz.reponse.toLowerCase() === collected.first().content.toLowerCase()) {
                if (find.xp + 10 == 100) {
                  const update = await QuizBD.findOneAndUpdate({ userID: message.author.id }, {
                    $set: {
                      xp: '0', level: find.level + 1, quizs: find.quizs + 1, Corrects: find.Corrects + 1,
                    },
                  }, { new: true });
                ...
                const update = await QuizBD.findOneAndUpdate({ userID: message.author.id }, {
                  $set: {
                    xp: find.xp + 10, level: find.level, quizs: find.quizs + 1, Corrects: find.Corrects + 1,
                  },
                }, { new: true });
                return message.succesMessage(`🏆 Bravo **${collected.first().author.username}** C'est la bonne réponse ! Tu as gagné \`10\`xp . !`);
              }
              const update = await QuizBD.findOneAndUpdate({ userID: message.author.id }, { $set: { quizs: find.quizs + 1 } }, { new: true });
