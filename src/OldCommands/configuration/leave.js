    ...
    message.channel.send(embed).then((m) => {
      ...
      m.awaitReactions(filtro, {
        ...
        if (reaccion.emoji.id === '830347712330203146') {
          ...
          if (verify) {
            const newchannel = await Welcome.findOneAndUpdate({ serverID: message.guild.id, reason: 'leave' }, {
              $set: {
                channelID: response.channel.id, reason: 'leave', message: response.message, status: response.status, image: response.image,
              },
            }, { new: true });
