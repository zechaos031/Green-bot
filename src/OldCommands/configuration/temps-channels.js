  ...
  async execute(message, args, client) {
    ...
    message.channel.send(embed).then((m) => {
      ...
      m.awaitReactions(filtro, {
        ...
        if (reaccion.emoji.id === '830347712330203146') {
          ...
          const check = await Temps.findOne({ serverID: message.guild.id });
          if (check) {
    ...
    async function getResponses(message) {
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        if (i === 0) {
          const channel = m.mentions.channels.first() || message.guild.channels.cache.get(content) || message.guild.channels.cache.filter((m) => m.type === 'voice' && m.name.includes(args.join(' '))).first();
        ...
        if (i === 1) {
          const channel = m.mentions.channels.first() || message.guild.channels.cache.get(content) || message.guild.channels.cache.filter((m) => m.name.includes(args.join(' '))).first();
