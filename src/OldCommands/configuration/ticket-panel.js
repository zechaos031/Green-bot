...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (args[0] === 'create') {
      ...
      const chekexeist = await ticketPanel.findOne({ ticketID: ID, serverID: message.guild.id });
      if (!chekexeist) return message.errorMessage(`je n'ai trouvé aucun panel de ticket avec pour id \`${ID}\``);
      const de = await ticketPanel.findOneAndDelete({ ticketID: ID, serverID: message.guild.id });
    ...
    async function getResponses(message) {
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        if (i === 0) {
          const channel = m.mentions.channels.first() || message.guild.channels.cache.get(content) || message.guild.channels.cache.filter((m) => m.type === 'text' && m.name.includes(content)).first();
        ...
        if (i === 1) {
          const role = m.mentions.roles.first() || message.guild.roles.cache.get(content);
        ...
        if (i === 4) {
          const channel = m.mentions.channels.first() || message.guild.channels.cache.get(content) || message.guild.channels.cache.filter((m) => m.type === 'category' && m.name.includes(content)).first();
