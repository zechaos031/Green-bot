  ...
  async execute(message, args, client) {
    ...
    message.channel.send(embed).then((m) => {
      ...
      m.awaitReactions(filtro, {
        ...
        if (reaccion.emoji.id === '830347712330203146') {
          ...
          if (verify) {
            const newchannel = await Welcome.findOneAndUpdate({ serverID: message.guild.id, reason: 'welcome' }, {
              $set: {
                channelID: response.channel.id, reason: 'welcome ', message: response.message, status: response.status, image: response.image,
              },
            }, { new: true });
    ...
    async function getResponses(message) {
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        if (i === 1) {
          const channel = m.mentions.channels.first() || message.guild.channels.cache.get(content) || message.guild.channels.cache.filter((m) => m.type === 'text' && m.name.includes(args.join(' '))).first();
