...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const Find = await Guild.findOne({ serverID: member.user.id, reason: 'rank_color' });
    const color = Find ? Find.content : '#3A871F';
    const Findx = await Guild.findOne({ serverID: member.user.id, reason: 'rank_xp_color' });
    const colorxp = Findx ? Findx.content : '#3A871F';
    const EditActive = await Guild.findOne({ serverID: message.guild.id, reason: 'level_edit' });
    let url;
