...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const prefixget = await guild.findOne({ serverID: message.guild.id, reason: 'prefix' }).sort();
    const prefix = prefixget.content;

    if (!args.length) {
      const customs = await CmdModel.find({ serverID: message.guild.id });

      const Title = await message.translate("Commande d'aide - Green-bot");

      const Description = await message.translate('● Vous pouvez me configurer depuis mon [Dashboard](http://green-bot.xyz/commands)');
      ...
      return message.channel.send(exampleEmbed).catch((error) => message.errorMessage('Une erreur est survenue , surement les permissions'));
    }

    const name = args[0].toLowerCase(); const
    ...
    const des = await message.translate(command.description);
