...
module.exports = {
  ...
  async execute(message, args) {
    const role = message.mentions.roles.first() || message.guild.roles.cache.get(args[0]) || message.guild.roles.cache.filter((m) => m.name.includes(args.join(' '))).first();
    if (!args.length) {
    ...
    const u = message.guild.members.cache.filter((u) => u.roles.cache.has(role.id));
