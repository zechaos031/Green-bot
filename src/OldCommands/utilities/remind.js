...
module.exports = {
  ...
  async execute(message, args, client) {
    if (!args[0]) return message.errorMessage('Veuillez fournir une durée de 14 jours ou moins (1s/m/h/d)');
    const time = ms(args[0]);
    if (!time || time > 1209600000) return message.errorMessage('Veuillez fournir une durée de 14 jours ou moins');
