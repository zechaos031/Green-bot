...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const channeldb = await ChannelModel.findOne({ serverID: message.guild.id, reason: 'sugg' });
    ...
    const suggs = await sugg.find({ serverID: message.guild.id });
    ...
    const paul = new Discord.MessageEmbed()
      ...
      .setColor('#F0F010');
    const suggc = message.guild.channels.cache.get(channeldb.content);
