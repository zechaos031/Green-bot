...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const commandOnSErver = await guild.find({ serverID: message.guild.id, reason: 'command' });
    const commands = await guild.find({ reason: 'command' });
    const currentGiveaways = message.client.manager.giveaways.length;
    const check = await backup.find({});
    const suggs = await sugg.find({});
